...
var _Node = class _Node {
  ...
  isolatedRender(renderCtx) {
    ...
    try {
      ...
      import_ag_charts_core4.Logger.warnOnce("Error during rendering", e, e.stack);
    } finally {
...
var logTimeGap = () => {
  ...
  if (timeSinceLastLog > LONG_TIME_PERIOD_THRESHOLD) {
    ...
    import_ag_charts_core5.Logger.log(`**** ${prettyDuration}s since last log message ****`);
  }
...
var Debug = {
  create(...debugSelectors) {
    const resultFn = (...logContent) => {
      if (Debug.check(...debugSelectors)) {
        if (typeof logContent[0] === "function") {
          logContent = (0, import_ag_charts_core5.toArray)(logContent[0]());
        }
        logTimeGap();
        import_ag_charts_core5.Logger.log(...logContent);
      }
    };
    return Object.assign(resultFn, {
      ...
      group: (name, cb) => {
        if (Debug.check(...debugSelectors)) {
          return import_ag_charts_core5.Logger.logGroup(name, cb);
        }
        return cb();
      }
    });
  ...
  check(...debugSelectors) {
    ...
    const chartDebug = (0, import_ag_charts_core5.toArray)((0, import_ag_charts_core5.getWindow)("agChartsDebug"));
...
var BaseProperties = class {
  ...
  set(properties) {
      ...
      if (keys.has(propertyKey)) {
        ...
        if (isProperties(self[propertyKey])) {
          if (self[propertyKey] instanceof PropertiesArray) {
            ...
            if (array == null) {
              import_ag_charts_core12.Logger.warn(`unable to set [${String(propertyKey)}] - expecting a properties array`);
            } else {
    ...
    for (const unknownKey of keys) {
      import_ag_charts_core12.Logger.warn(`unable to set [${String(unknownKey)}] in ${className} - property is unknown`);
    }
    return this;
  }
  clear() {
...
var Image2 = class {
  ...
  toSvg(bbox, pixelRatio) {
    ...
    const rect = (0, import_ag_charts_core14.createSvgElement)("rect");
...
function bezier2DDistance(cp0x, cp0y, cp1x, cp1y, cp2x, cp2y, cp3x, cp3y, x, y, precision = 1) {
  ...
  while (queue != null) {
    ...
    queue = (0, import_ag_charts_core16.insertListItemsSorted)(queue, newCandidates, bezierCandidateCmp);
  }
...
function isImageFill(fill) {
  return fill !== null && (0, import_ag_charts_core18.isObject)(fill) && fill.type == "image";
}
...
var Shape = class extends Node {
  ...
  applySvgStrokeAttributes(element) {
    ...
    setSvgStrokeAttributes(element, { stroke: (0, import_ag_charts_core19.isString)(stroke) ? stroke : void 0, strokeOpacity, strokeWidth });
...
__decorateClass([
  SceneChangeDetection({ convertor: (v) => (0, import_ag_charts_core19.clamp)(0, v ?? 1, 1) })
...
__decorateClass([
  SceneObjectChangeDetection({ equals: import_ag_charts_core19.boxesEqual, changeCb: (s) => s.onFillChange() })
...
__decorateClass([
  SceneChangeDetection({ convertor: (v) => (0, import_ag_charts_core22.clamp)(0, v, 1) })
...
function debugStats(layersManager, debugSplitTimes, ctx, renderCtxStats, extraDebugStats = {}, seriesRect = BBox.zero) {
  ...
  ].filter(import_ag_charts_core23.isString);
  const measurer = new import_ag_charts_core23.TextMeasurer(ctx);
  ...
  try {
    ...
    import_ag_charts_core23.Logger.warnOnce("Error during debug stats rendering", e);
  } finally {
    ctx.restore();
  }
}
function prepareSceneNodeHighlight(ctx) {
  const config = (0, import_ag_charts_core23.toArray)((0, import_ag_charts_core23.getWindow)("agChartsSceneDebug"));
  const result = [];
...
function debugSceneNodeHighlight(ctx, debugNodes) {
  ...
  try {
    for (const [name, node] of Object.entries(debugNodes)) {
      ...
      if (!bbox) {
        import_ag_charts_core23.Logger.log(`Scene.render() - no bbox for debugged node [${name}].`);
        continue;
      }
      ctx.globalAlpha = 0.8;
      ctx.strokeStyle = "red";
    ...
    import_ag_charts_core23.Logger.warnOnce("Error during debug rendering", e);
  } finally {
    ctx.restore();
  }
}
...
function time(name, start, end2) {
  const duration = end2 != null ? end2 - start : start;
...
var Path = class extends Shape {
  ...
  toSVG() {
    if (!this.visible)
      return;
    const element = (0, import_ag_charts_core24.createSvgElement)("path");
...
var cornerEdges = (leadingEdge, trailingEdge, leadingInset, trailingInset, cornerRadius) => {
  ...
  } else if ((0, import_ag_charts_core25.isNumberEqual)(leading0, 0)) {
    leading0 = 0;
  }
  ...
  } else if ((0, import_ag_charts_core25.isNumberEqual)(trailing1, 0)) {
    trailing1 = 0;
  }
...
__decorateClass([
  SceneObjectChangeDetection({ equals: import_ag_charts_core25.boxesEqual })
...
var _Text = class _Text extends Shape {
  ...
  onTextChange() {
    this.richText?.clear();
    this.textMap?.clear();
    if ((0, import_ag_charts_core26.isArray)(this.text)) {
      this.lines = [];
      this.richText ?? (this.richText = new Group());
      this.richText.setScene(this.scene);
      this.richText.append(
        this.text.flatMap((s) => s.text.split(import_ag_charts_core26.LineSplitter)).filter(Boolean).map(() => new _Text({ trimText: false }))
      );
    } else {
  ...
  get font() {
    this.fontCache ?? (this.fontCache = (0, import_ag_charts_core26.toFontString)(this));
    return this.fontCache;
  }
  static computeBBox(lines, x, y, opts) {
    const { font, lineHeight, textAlign, textBaseline } = opts;
    const { width, height, lineMetrics } = (0, import_ag_charts_core26.cachedTextMeasurer)(font).measureLines(lines);
    const totalHeight = lineHeight ? lineHeight * lineMetrics.length : height;
    const offsetTop = _Text.calcTopOffset(totalHeight, lineMetrics, textBaseline);
    const offsetLeft = _Text.calcLeftOffset(width, textAlign);
    return new BBox(x - offsetLeft, y - offsetTop, width, totalHeight);
  }
  static calcTopOffset(height, lineMetrics, textBaseline) {
    switch (textBaseline) {
      case "alphabetic":
  ...
  getPlainText() {
    return (0, import_ag_charts_core26.toPlainText)(this.text);
  }
  isPointInPath(x, y) {
  ...
  generateTextMap() {
    if (!(0, import_ag_charts_core26.isArray)(this.text) || this.textMap?.size)
      return;
    this.textMap ?? (this.textMap = /* @__PURE__ */ new Map());
    let offsetY = 0;
    const textNodes = this.richText.children();
    for (const { width, height, ascent, segments } of (0, import_ag_charts_core26.measureTextSegments)(this.text, this).lineMetrics) {
      let offsetX = 0;
  ...
  renderLines(renderCallback) {
    ...
    if (!Number.isFinite(x) || !Number.isFinite(y))
      return;
    const measurer = (0, import_ag_charts_core26.cachedTextMeasurer)(this);
  ...
  toSVG() {
    if (!this.visible || !this.text)
      return;
    const element = (0, import_ag_charts_core26.createSvgElement)("text");
    if ((0, import_ag_charts_core26.isString)(this.text)) {
      ...
      for (const segment of this.text) {
        const segmentElement = (0, import_ag_charts_core26.createSvgElement)("tspan");
...
var Caption = class extends BaseProperties {
  constructor() {
    super(...arguments);
    this.id = (0, import_ag_charts_core28.createId)(this);
  ...
  computeTextWrap(containerWidth, containerHeight) {
    ...
    let wrappedText;
    if ((0, import_ag_charts_core28.isArray)(text)) {
      wrappedText = (0, import_ag_charts_core28.wrapTextSegments)(text, options);
      this.truncated = wrappedText.some(import_ag_charts_core28.isSegmentTruncated);
    } else {
      wrappedText = (0, import_ag_charts_core28.wrapText)(text ?? "", options);
      this.truncated = (0, import_ag_charts_core28.isTextTruncated)(wrappedText);
    }
  ...
  updateA11yText(moduleCtx, where) {
    ...
    if (this.enabled && this.text) {
      ...
      if (bbox) {
        ...
        this.proxyText.textContent = (0, import_ag_charts_core28.toPlainText)(this.text);
  ...
  handleMouseMove(moduleCtx, event) {
    if (event != null && this.enabled && this.truncated) {
      ...
      moduleCtx.tooltipManager.updateTooltip(this.id, { canvasX, canvasY, showArrow: false }, [
        { type: "structured", title: (0, import_ag_charts_core28.toPlainText)(this.text) }
      ]);
    }
  }
...
var _BandScale = class _BandScale extends AbstractScale {
  ...
  set padding(value) {
    value = (0, import_ag_charts_core30.clamp)(0, value, 1);
  ...
  set paddingInner(value) {
    this.invalid = true;
    this._paddingInner = (0, import_ag_charts_core30.clamp)(0, value, 1);
  }
  get paddingInner() {
    return this._paddingInner;
  }
  set paddingOuter(value) {
    this.invalid = true;
    this._paddingOuter = (0, import_ag_charts_core30.clamp)(0, value, 1);
  }
  get paddingOuter() {
  ...
  refresh() {
    ...
    this.update();
    if (this.invalid) {
      import_ag_charts_core30.Logger.warnOnce("Expected update to not invalidate scale");
    }
  }
  convert(d, options) {
    this.refresh();
  ...
  ordinalRange(i) {
    ...
    return (0, import_ag_charts_core30.clamp)(min, inset + step * i, max);
  }
};
__decorateClass([
  Invalidating
...
function visibleTickRange(ticks, reversed, visibleRange) {
  if (visibleRange == null || visibleRange[0] === 0 && visibleRange[1] === 1)
    return;
  const vt0 = (0, import_ag_charts_core31.clamp)(0, Math.floor(visibleRange[0] * ticks.length), ticks.length);
  const vt1 = (0, import_ag_charts_core31.clamp)(0, Math.ceil(visibleRange[1] * ticks.length), ticks.length);
...
var CategoryScale = class _CategoryScale extends BandScale {
  ...
  ticks(params, domain = this.domain, visibleRange) {
    ...
    const vt0 = (0, import_ag_charts_core32.clamp)(0, Math.floor((visibleRange?.[0] ?? 0) * bands.length), bands.length);
    const vt1 = (0, import_ag_charts_core32.clamp)(0, Math.ceil((visibleRange?.[1] ?? 1) * bands.length), bands.length);
...
// packages/ag-charts-community/src/util/visibleRange.ts
...
function tickStep(start, end2, count, minCount = 0, maxCount = Infinity) {
  if (start === end2) {
    return (0, import_ag_charts_core33.clamp)(1, minCount, maxCount);
  } else if (count < 1) {
    return NaN;
  }
  const extent = Math.abs(end2 - start);
...
function range(start, end2, step, visibleRange) {
  if (!Number.isFinite(step) || step <= 0) {
    return { ticks: [], count: 0, firstTickIndex: void 0 };
  } else if (start === end2) {
    return { ticks: [start], count: 1, firstTickIndex: 0 };
  }
  const f = 10 ** (0, import_ag_charts_core33.countFractionDigits)(step);
  const d0 = Math.min(start, end2);
  const d1 = Math.max(start, end2);
  let vd0;
  let vd1;
  if (visibleRange != null && (visibleRange[0] !== 0 || visibleRange[1] !== 1)) {
    const rangeExtent = end2 - start;
...
function isDenseInterval(count, availableRange) {
  if (count >= availableRange) {
    import_ag_charts_core33.Logger.warnOnce(
...
function niceTicksDomain(start, end2) {
  const extent = Math.abs(end2 - start);
  const step = 10 ** Math.floor(Math.log10(extent));
  let minError = Infinity, ticks = [start, end2];
  for (const multiplier of TickMultipliers) {
...
var HdpiCanvas = class {
  constructor(options) {
    ...
    this.pixelRatio = options.pixelRatio ?? (0, import_ag_charts_core34.getWindow)("devicePixelRatio") ?? 1;
    this.element = canvasElement ?? (0, import_ag_charts_core34.createElement)("canvas");
...
var ImageLoader = class extends import_ag_charts_core35.EventEmitter {
  constructor() {
    super(...arguments);
...
var Arc = class extends Path {
  ...
  get fullPie() {
    return (0, import_ag_charts_core37.isNumberEqual)(normalizeAngle360(this.startAngle), normalizeAngle360(this.endAngle));
  }
  updatePath() {
...
var RadialColumnShape = class extends Path {
  ...
  updateBeveledPath() {
    ...
    const isStackBottom = (0, import_ag_charts_core39.isNumberEqual)(innerRadius, axisInnerRadius);
    ...
    const isEmpty = (0, import_ag_charts_core39.isNumberEqual)(innerRadius, outerRadius);
    ...
    if (isEmpty && shouldConnectBottomCircle) {
      ...
      if (!(0, import_ag_charts_core39.isNumberEqual)(topIntersectionX, 0)) {
